...
export default {
  setup() {
    ...
    const fetchMenus = async () => {
      try {
        const response = await fetch('http://192.168.1.10/menu/history')
        if (!response.ok) {
    ...
    const addMenu = async () => {
      try {
        ...
        const response = await fetch('http://192.168.1.10/menu', {
          method: 'POST',
    ...
    async function confirmUpdate() {
      ...
      for (const rowId in changesMap.value) {
        ...
        const url = `http://192.168.1.10/menu/fresh`
      ...
      try {
        ...
        if (confirmDelete) {
          const response = await fetch(
            `http://192.168.1.10/menu/delete?datum=${menu.Datum_marende}`,
            {
              method: 'DELETE',
            },
          )
...
/* All API endpoints have been updated to use http://192.168.1.10 */
