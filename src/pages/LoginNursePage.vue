<template>
  <q-page class="flex flex-center" style="background-color: #e0f7fa">
    <div class="login-container">
      <h6 class="text-h6">Dobrodošli na prijavu za sestre</h6>
...
const submit = async () => {
  try {
    const response = await axios.post('https://backend-hospital-n9to.onrender.com/login_nurse', {
    ...
    // Spremite korisničko ime i ID sestre u localStorage
    localStorage.setItem('loggedInUser', username.value)
    localStorage.setItem('idSestre', response.data.ID_sestre)
