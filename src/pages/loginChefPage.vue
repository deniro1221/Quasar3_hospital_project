<template>
  <q-page class="flex flex-center" style="background-color: #e0f7fa">
    <div class="login-container">
      <h6 class="text-h6">Dobrodošli na prijavu za kuhare</h6>
...
const submit = async () => {
  try {
    const response = await axios.post('https://backend-hospital-n9to.onrender.com/login_chef', {
      username: username.value,
      password: password.value,
    })

    console.log(response.data.message) // Poruka od servera
    errorMessage.value = '' // Reset greške

    // Spremite korisničko ime i ID kuhaara u localStorage
