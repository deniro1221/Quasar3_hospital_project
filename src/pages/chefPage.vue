...
const fetchChefs = async () => {
  try {
    const response = await axios.get('http://localhost:3003/chef')
...
const addChef = async () => {
  try {
    await axios.post('http://localhost:3003/chef', form.value)
    await fetchChefs()
    closeDialog()
  } catch (error) {
...
const updateChef = async () => {
  try {
    await axios.put(`http://localhost:3003/chef/${form.value.ID_kuhara}`, form.value)
    await fetchChefs()
    closeDialog()
  } catch (error) {
...
const deleteChef = async (id) => {
  if (confirm('Jeste li sigurni da želite izbrisati ovog kuhara?')) {
    try {
      await axios.delete(`http://localhost:3003/chef/${id}`)
      await fetchChefs()
    } catch (error) {
